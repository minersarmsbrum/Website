...
export function AdminSidebar({ username }: { username: string }) {
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center gap-3 border-b border-cream-200/10 px-5 py-5">
        ...
        <div className="min-w-0">
          ...
          <p className="text-[10px] uppercase tracking-luxe text-saffron-500/70">Admin Portal</p>
        </div>
      </div>
      ...
      <div className="border-t border-cream-200/10 px-5 py-4">
        <p className="mb-3 truncate text-xs text-cream-200/40">Signed in as <span className="text-cream-200/70">{username}</span></p>
        <button
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden w-60 shrink-0 border-r border-cream-200/10 bg-ink-800/80 backdrop-blur lg:block">
        {sidebarContent}
      </aside>
      ...
      <div className="fixed inset-x-0 top-0 z-40 flex h-14 items-center justify-between border-b border-cream-200/10 bg-ink-900/95 px-4 backdrop-blur lg:hidden">
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-display text-sm text-cream-50">Admin</span>
        </div>
        <button
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-30 lg:hidden">
          ...
          <aside className="absolute left-0 top-14 bottom-0 w-60 border-r border-cream-200/10 bg-ink-800">
            {sidebarContent}
          </aside>
        </div>
      )}
      ...
      {/* Mobile spacer */}
      <div className="h-14 lg:hidden" style={{ display: "none" }} />
    </>
  );
}
